...
if not OPENAI_API_KEY or not OPENAI_API_BASE:
    raise ValueError(
        "\n" + "="*70 + "\n"
        "❌ 错误：未找到 OPENAI_API_KEY / OPENAI_API_BASE 环境变量！\n"
        "="*70 + "\n"
        "请按照以下步骤设置环境变量：\n\n"
        "1️⃣ 访问 https://platform.openai.com/api-keys 获取 API 密钥\n"
        "2️⃣ 复制 .env.example 为 .env\n"
        "   命令：cp .env.example .env\n"
        "3️⃣ 在 .env 文件中填入你的 OpenAI 配置：\n"
...
# 验证 API 密钥格式（OpenAI API key 通常以 sk- 开头）
if not OPENAI_API_KEY.startswith("sk-"):
    print("\n" + "⚠️  警告：你的 OPENAI_API_KEY 格式可能不正确")
    print("   OpenAI API 密钥通常以 'sk-' 开头")
    print("   请确认你从 https://platform.openai.com/api-keys 获取了正确的密钥\n")
